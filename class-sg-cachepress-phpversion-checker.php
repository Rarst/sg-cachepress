...
class SG_CachePress_PHPVersionChecker
{
    ...
    public function global_notice_template($msg, $id)
    {
        if ($this->options_handler->is_enabled('show_notice_' . $id)) {                            
            $html = '<div id="ajax-'.$id.'" class="updated sg-cachepress-notification">';
            $html .= '<p>';
            $html .= __('<strong>SG CachePress PHP:</strong>'
                    . $msg . ' Click <a href="http://www.siteground.com/tutorials/supercacher/" target="_blank">here</a> for mode details. '
                    . '<br /> <a href="javascript:;" id="'.$id.'" class="dismiss-sg-cahepress-notification-by-id">Click here to hide this notice</a>.', 'ajax-notification');
            $html .= '</p>';
